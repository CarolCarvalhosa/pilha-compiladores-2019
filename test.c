...
void push(Pilha * stk){
	if(!pilhaCheia()){
        pilha[fimPilha].simb = "a"; //stk.simb;
		pilha[fimPilha].atributo = "t"; //stk.atributo;
		pilha[fimPilha].estado = 1; //stk.estado;
		fimPilha++;
	}else{
        printf("\nPilha cheia\n");
	}
